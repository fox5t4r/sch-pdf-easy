...
LMS에 로그인된 페이지가 열리면 별도 강의 페이지에 들어가지 않아도 background에서 수강 과목 목록을 조회하고, 각 과목의 Canvas 파일 API와 LearningX 자료 API를 순회해 접근 가능한 PDF/PPT/PPTX만 자동으로 다운로드합니다.

자동 백업은 이미 다운로드 기록이 있는 파일을 다시 받지 않으며, 같은 로그인 세션에서 반복 실행되지 않도록 10분 쿨다운을 둡니다. 대량 요청을 피하기 위해 기본 동시 다운로드는 2개로 제한하고, 서버가 숨김/비공개로 내려준 자료만 자동 백업에서 제외합니다.
