...
LMS가 제공하는 공개 시작/종료 시각(`unlock_at`, `lock_at` 등)과 잠금/숨김 상태를 탐지할 수 있으면 패널에 “n시간 남음”, “기간 종료”, “접근 제한” 같은 배지를 표시합니다.

확장 프로그램은 현재 로그인 세션에서 정상 접근 가능한 자료만 다운로드 대상으로 삼습니다. 공개 예정, 기간 종료, 잠금, 숨김 상태로 확인된 자료는 “새 파일”과 “전체” 다운로드 대상에서 제외하며, 접근 제한을 우회하거나 만료된 URL을 강제로 호출하지 않습니다.

따라서 마감 임박 배지는 사용자가 미리 백업할 수 있도록 알려주는 보조 정보이며, 실제 접근 가능 여부는 LMS의 인증과 권한 응답을 최종 기준으로 따릅니다.
...
자동 백업은 이미 다운로드 기록이 있는 파일을 다시 받지 않으며, 같은 로그인 세션에서 반복 실행되지 않도록 10분 쿨다운을 둡니다. 대량 요청을 피하기 위해 기본 동시 다운로드는 2개로 제한하고, 기간 종료/잠금/숨김/공개 예정 자료는 자동 백업에서도 제외합니다.
