...
다운로드 시작 지연을 줄이기 위해 스캔 후 새 파일의 다운로드 URL을 제한된 동시성으로 미리 조회하고, 실제 다운로드 요청은 background에서 시작 즉시 응답한 뒤 완료/실패 이벤트로 UI와 기록을 갱신합니다. 기본 동시 다운로드 수는 5개이며, 내부 설정값 `downloadConcurrency`는 1~8 범위로 제한됩니다.

### 4. 공개 기간 표시와 백업 정책

LMS가 제공하는 공개 시작/종료 시각(`unlock_at`, `lock_at` 등)과 잠금/숨김 상태를 탐지할 수 있으면 패널에 “n시간 남음”, “기간 종료”, “접근 제한 표시”, “서버 숨김” 같은 배지를 표시합니다.

공개 예정, 기간 종료, 잠금 상태는 LMS UI의 안내 메타데이터로만 취급합니다. 실제로 서버가 파일 목록에 자료를 내려주고 다운로드 URL 조회를 허용하면 “새 파일”, “전체”, 자동 백업 대상에 포함합니다.

확장 프로그램이 선제적으로 제외하는 경우는 서버 응답에서 숨김/비공개 상태가 명확한 자료뿐입니다. 그 외에는 정상 로그인 세션으로 다운로드를 시도하고, 서버가 실제로 URL 조회나 파일 다운로드를 거부하면 실패로 처리합니다.

### 5. 로그인 후 자동 백업

LMS에 로그인된 페이지가 열리면 별도 강의 페이지에 들어가지 않아도 background에서 수강 과목 목록을 조회하고, 각 과목의 Canvas 파일 API와 LearningX 자료 API를 순회해 접근 가능한 PDF/PPT/PPTX만 자동으로 다운로드합니다. API만으로 강의콘텐츠 자료를 찾지 못하는 경우에는 과목별 강의콘텐츠 페이지를 비활성 탭으로 잠시 열어 페이지 내부 자료 목록을 스캔한 뒤 자동으로 닫습니다.

자동 백업은 이미 다운로드 기록이 있는 파일을 다시 받지 않으며, 같은 로그인 세션에서 반복 실행되지 않도록 10분 쿨다운을 둡니다. 대량 요청을 피하기 위해 기본 동시 다운로드는 2개로 제한하고, 서버가 숨김/비공개로 내려준 자료만 자동 백업에서 제외합니다. 비활성 탭 순회 때문에 `tabs` 권한을 사용하지만, 탭 내용은 LMS 자료 스캔과 완료 후 닫기 용도로만 사용합니다.
...
├── auto_sync.js       # 로그인 감지 후 전체 과목 자동 백업 트리거
...
2. 로그인된 LMS 페이지가 열리면 새 자료 자동 백업이 background에서 시작됩니다.
3. 특정 강의에서 직접 확인하려면 강의콘텐츠 또는 강의자료실 페이지에 접속합니다.
4. 우측 하단 PDF 버튼을 클릭합니다.
5. 패널이 열리면 해당 강의 자료 스캔이 시작됩니다.
6. 새 파일만 받으려면 “새 파일”을 클릭합니다.
7. 전체 파일을 다시 받으려면 “전체”을 클릭합니다.
8. 파일은 아래 경로에 저장됩니다.
...
* 확장 프로그램은 0KB가 발생하기 쉬운 임시 `content_download_uri` 대신 `content_id`와 확장자로 Commons 원본 파일 다운로드 엔드포인트를 직접 구성해 사용합니다.
* 다운로드 완료 후에도 파일 크기가 0바이트로 확인되면 빈 파일을 지우고 성공 기록을 남기지 않습니다.
